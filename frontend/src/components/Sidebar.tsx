    ...
    <>
      ...
      <div className="hidden lg:flex w-full bg-gradient-to-b from-black-200/40 to-white-400/40 backdrop-blur-lg border-r border-white/10 flex-col items-center py-8 gap-8 h-full">
        ...
        <nav className="flex flex-col gap-6 flex-1 w-full items-center">
          ...
          {/* === HOME (PC) === */}
          {/* Linkで囲んで、中のdiv(button)のクラスを条件分岐させる */}
          <Link to="/">
            <div className={`w-12 h-12 rounded-lg transition-all duration-300 flex items-center justify-center cursor-pointer shadow-inner
              ${isActive('/')
                ? 'bg-blue-500/20 text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-110'
                : 'bg-slate-700/30 text-slate-300 hover:bg-slate-600/40 hover:text-white hover:scale-110'
              }`}
            >
              <Home size={24} />
            </div>
          ...
          {/* === SETTINGS (PC) === */}
          <Link to="/settings">
            <div className={`w-12 h-12 rounded-lg transition-all duration-300 flex items-center justify-center cursor-pointer
              ${isActive('/settings')
                ? 'bg-blue-500/20 text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.5)] scale-110'
                : 'bg-slate-700/30 text-slate-300 hover:bg-slate-600/40 hover:text-white hover:scale-110'
              }`}
      ...
      <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md z-50">
        <div className="flex justify-around items-center bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-full px-4 py-3 shadow-2xl">
          ...
          {/* === HOME (Mobile) === */}
          <Link to="/" className={`p-2 transition-transform flex flex-col items-center 
            ${isActive('/') ? 'text-blue-400 -translate-y-1' : 'text-slate-300 hover:text-white active:scale-95'}`}
          >
            <div className={`p-2 rounded-lg transition-all duration-300 
              ${isActive('/') ? 'bg-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-slate-700/30'}`}
            >
              <Home size={24} />
            </div>
          </Link>
          ...
          {/* === SETTINGS (Mobile) === */}
          {/* ここを isActive('/') から isActive('/settings') に修正しました！ */}
          <Link to="/settings" className={`p-2 transition-transform flex flex-col items-center 
            ${isActive('/settings') ? 'text-blue-400 -translate-y-1' : 'text-slate-300 hover:text-white active:scale-95'}`}
          >
            <div className={`p-2 rounded-lg transition-all duration-300 
              ${isActive('/settings') ? 'bg-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-slate-700/30'}`}
            >
              <Settings size={24} />
            </div>
          </Link>
